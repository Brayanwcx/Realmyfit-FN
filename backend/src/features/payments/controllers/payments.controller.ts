...
export class PaymentsController {
    ...
    /** Creates a Stripe Checkout session specifically for Events */
    @ApiBearerAuth()
    @UseGuards(JwtAuthGuard)
    @Post('stripe/checkout-session/event')
    @ApiOperation({ summary: 'Create a Stripe Checkout session for Events' })
    @ApiResponse({ status: 201, description: 'Returns { url, sessionId }' })
    createEventCheckoutSession(@Body() dto: CreateEventCheckoutDto) {
