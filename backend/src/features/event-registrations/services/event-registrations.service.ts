...
export class EventRegistrationsService {
    ...
    private getActiveRegistrations(event: Event) {
        return (event.registrations || []).filter((r) => r.status !== RegistrationStatus.CANCELLED);
    }
    ...
    async create(dto: CreateEventRegistrationDto) {
        ...
        if (Number(event.price) > 0) {
            throw new BadRequestException('Este evento requiere pago. Completa el proceso de compra.');
        }
