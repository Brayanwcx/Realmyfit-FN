...
export class PaymentService {
  ...
  createEventCheckoutSession(
    ...
    return this.http.post<CheckoutSessionResponse>(
      `${this.apiUrl}/payments/stripe/checkout-session/event`,
      { eventId, userId, successUrl, cancelUrl }
    );
  }

  verifyCheckoutSession(sessionId: string): Observable<{ success: boolean; status: string; orderId?: number; type?: string; eventId?: number }> {
