  ...
  template: `
    <div class="view-header">
      ...
      <div class="header-stats">
        ...
        <span class="stat-chip refund">Reembolso Pendiente: {{ countByStatus('PENDING_REFUND') }}</span>
      </div>
    </div>
    ...
    <div class="filters-row">
      ...
      <select [(ngModel)]="statusFilter" (ngModelChange)="page = 1" class="filter-select">
        ...
        <option value="PENDING_REFUND">Reembolso Pendiente</option>
      </select>
    </div>
                ...
                <td>
                  <div class="actions-cell">
                    ...
                    @if (payment.status === 'PENDING_REFUND') {
                      <button class="btn-icon btn-refund" (click)="markAsRefunded(payment)" title="Marcar como Reembolsado">
...
export class AdminOrdersComponent implements OnInit {
  ...
  private mergePaymentsWithOrders(payments: any[], orders: any[]) {
    ...
    return payments.map((payment) => {
      ...
      const isNonProduct = desc.includes('evento') || desc.includes('membresía') || desc.includes('membership');
  ...
  markAsRefunded(payment: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        http.patch(`${apiUrl}/payments/${payment.id}/refunded`, {}).subscribe({
          next: () => {
