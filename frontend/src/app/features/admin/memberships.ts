    ...
    <div class="table-container glass">
      ...
      @if (!loading && !errorMessage && memberships.length > 0) {
        ...
        <table class="desktop-table">
          ...
          <tbody>
            @for (membership of pagedMemberships; track membership.id) {
              <tr>
                <td>
        ...
        @if (totalPages > 1) {
          <div class="pagination">
            ...
            <span class="page-info">{{ (page-1)*pageSize+1 }}–{{ min(page*pageSize, memberships.length) }} de {{ memberships.length }}</span>
          </div>
        }
      }
